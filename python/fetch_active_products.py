...
from gen_utils import database_connection, get_api_key, get_search_words, get_test_search_words, get_trace_and_log, csv_reader
from gen_utils import ALPHA_PATH, BETA_PATH, UNLIMITED_PATH, REVISED_PATH, LEGENDS_PATH, ARABIAN_NIGHTS_PATH, DARK_PATH, ANTIQUITIES_PATH, HOMELANDS_PATH, FALLEN_EMPIRES_PATH, COLLECTORS_EDITION_PATH, INTERNATIONAL_COLLECTORS_EDITION_PATH
...
if __name__ == '__main__':
    ...
    # PATHS = [ALPHA_PATH, BETA_PATH, UNLIMITED_PATH]
    # alphaWords = csv_reader(ALPHA_PATH)
    # betaWords = csv_reader(BETA_PATH)
    # unlWords = csv_reader(UNLIMITED_PATH)
    # revWords = csv_reader(REVISED_PATH)
    # legWords = csv_reader(LEGENDS_PATH)
    # arnWords = csv_reader(ARABIAN_NIGHTS_PATH)
    # darkWords = csv_reader(DARK_PATH)
    # atqWords = csv_reader(ANTIQUITIES_PATH)
    # homeWords = csv_reader(HOMELANDS_PATH)
    # femWords = csv_reader(FALLEN_EMPIRES_PATH)
    # ceWords = csv_reader(COLLECTORS_EDITION_PATH)
    # iceWords = csv_reader(INTERNATIONAL_COLLECTORS_EDITION_PATH)
    # words = alphaWords + betaWords + unlWords + revWords + legWords + arnWords + darkWords + atqWords + homeWords + femWords + ceWords + iceWords
    # words = ["Collector's Edition Black Lotus MTG", "International Edition Mox Ruby MTG", "Beta Black Lotus MTG"]
    words = ["Revised Savannah MTG"]
    count = 0
    for word in words:
        # print(word)
        count += 1
